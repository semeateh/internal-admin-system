...
export default {
  ...
  computed: {
    departments() {
      const names = ["全部", ...new Set(this.templates.map(item => item.department))];
      return names.map(name => ({
        name,
        count: name === "全部" ? this.templates.length : this.templates.filter(item => item.department === name).length
      }));
    },
    visibleDepartments() {
      const query = this.departmentQuery.trim().toLowerCase();
      return this.departments.filter(item => !query || item.name.toLowerCase().includes(query));
    },
    filteredTemplates() {
      const query = this.templateQuery.trim().toLowerCase();
      return this.templates.filter(item => {
        const deptOk = this.selectedDepartment === "全部" || item.department === this.selectedDepartment;
    ...
    recentRows() {
      return this.instances.filter(item => this.selectedDepartment === "全部" || item.department === this.selectedDepartment);
    }
  },
  watch: {
    filteredTemplates() {
