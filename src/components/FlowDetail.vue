  ...
  <section>
    ...
    <main class="workspace">
      ...
      <el-card class="panel-card">
        <div slot="header" class="card-header">
          ...
          <div class="page-actions">
            ...
            <el-button v-if="isDone(currentStep) && !editingDone" icon="el-icon-edit" @click="editingDone = true">修改已完成内容</el-button>
            <el-button v-if="isDone(currentStep) && editingDone" type="primary" icon="el-icon-check" @click="saveRevision">保存修订</el-button>
            <el-button v-if="!isDone(currentStep)" type="success" icon="el-icon-document-checked" :disabled="currentStep.status !== 'current' || hasMissingRequired" @click="completeCurrentStep">完成当前任务</el-button>
          </div>
        </div>
        ...
        <div class="form-section">
          ...
          <el-form label-position="top" size="medium" class="form-grid">
            <el-form-item v-for="field in currentStep.fields" :key="field.key || field.id" :label="`${field.label}${field.required ? ' *' : ''}`" :class="{ wide: field.type === 'textarea' || field.type === 'choice' || field.type === 'select' }">
              <el-radio-group v-if="field.type === 'choice' || field.type === 'select'" v-model="field.value" :disabled="locked" @change="persistCurrentFields">
                <el-radio-button v-for="option in field.options || []" :key="option" :label="option" />
              </el-radio-group>
              <el-input v-else-if="field.type === 'textarea'" v-model="field.value" type="textarea" :disabled="locked" @change="persistCurrentFields" />
              <el-date-picker v-else-if="field.type === 'date'" v-model="field.value" value-format="yyyy-MM-dd" :disabled="locked" @change="persistCurrentFields" />
              <el-input v-else v-model="field.value" :disabled="locked" :placeholder="`请输入${field.label}`" @change="persistCurrentFields" />
            </el-form-item>
          </el-form>
        </div>
        ...
        <div class="form-section">
          ...
          <div class="assignment-grid">
            <el-select v-model="assignMode" :disabled="locked" @change="refreshAssignee">
              <el-option label="指定人员" value="user" />
              <el-option label="指定小组" value="group" />
            </el-select>
            <el-select v-model="assignee" filterable :disabled="locked">
              <el-option v-for="name in assigneeOptions" :key="name" :label="name" :value="name" />
            </el-select>
            <el-select v-model="currentStep.rule" :disabled="locked">
              <el-option label="任一人完成" value="any" />
              <el-option label="全部完成" value="all" />
            </el-select>
            <el-button type="primary" icon="el-icon-check" :disabled="locked" @click="applyAssignment">应用分配</el-button>
          </div>
        </div>
        ...
        <div v-if="currentStep.tasks.length" class="form-section">
          ...
          <div class="task-list">
            <div v-for="(task, index) in currentStep.tasks" :key="`${task.title}-${task.owner}`" class="task-row">
              ...
              <el-button size="mini" :disabled="task.status === 'done' || currentStep.status !== 'current' || hasMissingRequired" @click="completeTask(index)">完成</el-button>
            </div>
          </div>
        </div>
      </el-card>
      ...
      <aside class="side-stack">
        ...
        <el-card class="panel-card">
          <div slot="header"><h2 class="panel-title">通知与操作日志</h2><p class="panel-subtitle">模拟微信机器人通知</p></div>
          <div class="log-list">
...
<script>
import { completeInstanceStep, completeInstanceTask, remindInstance, updateInstanceStepFields } from "../api/instances.js";
...
export default {
  ...
  methods: {
    ...
    applyAssignment() {
      ...
      step.assignees = this.assignMode === "group" ? this.groups[this.assignee].slice() : [this.assignee];
      step.tasks = this.assignMode === "group"
        ? step.assignees.map(owner => ({ title: `${step.name}确认`, owner, status: "pending", note: "重新分配后生成的会签任务" }))
        : [];
