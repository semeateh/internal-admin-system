...
def complete_task(instance_id, task_id):
    ...
    return api_success(flow_service.complete_task(instance_id, task_id, g.current_user["id"], can_manage), "任务已完成。", "TASK_COMPLETED")
...
def complete_step(instance_id, step_id):
    can_manage = has_permission(g.current_user, "flow.template.manage")
    return api_success(flow_service.complete_step(instance_id, step_id, g.current_user["id"], can_manage), "步骤已完成。", "STEP_COMPLETED")
...
def remind(instance_id):
    ...
    return api_success(flow_service.remind_instance(instance_id, payload.get("message"), g.current_user["id"]), "催办已记录。", "REMIND_RECORDED")


@tasks_bp.get("/<int:instance_id>/logs")
@require_permission("flow.instance.view")
def logs(instance_id):
    instance = flow_service.get_instance(instance_id)
    if not instance:
