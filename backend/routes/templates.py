...
def list_templates():
    return api_success({"items": flow_service.list_templates()}, "模板列表已获取。", "TEMPLATE_LIST_OK")


@templates_bp.post("")
@require_permission("flow.template.manage")
def create_template():
    template = flow_service.create_template(request.get_json(silent=True) or {}, g.current_user["id"])
...
def get_template(template_id):
    template = flow_service.get_template(template_id)
    if not template:
...
def update_template(template_id):
    template = flow_service.update_template(template_id, request.get_json(silent=True) or {}, g.current_user["id"])
...
def add_step(template_id):
    template = flow_service.add_template_step(template_id, request.get_json(silent=True) or {}, g.current_user["id"])
...
def update_step(template_id, step_id):
    ...
    template = flow_service.update_template_step(template_id, step_id, payload, g.current_user["id"])
...
def delete_step(template_id, step_id):
    template = flow_service.delete_template_step(template_id, step_id, g.current_user["id"])
