...
def list_instances():
    ...
    return api_success({"items": flow_service.list_instances(template_id)}, "流程实例列表已获取。", "INSTANCE_LIST_OK")
...
def create_instance():
    ...
    instance = flow_service.create_instance(
...
def update_instance(instance_id):
    instance = flow_service.update_instance(instance_id, request.get_json(silent=True) or {}, g.current_user["id"])
    if not instance:
        return api_error("流程实例不存在。", 404, "INSTANCE_NOT_FOUND")
...
def update_instance_step_fields(instance_id, step_id):
    ...
    instance = flow_service.update_instance_step_fields(
        instance_id,
        step_id,
