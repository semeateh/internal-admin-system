...
def _template_status(value):
    return {1: "active", 0: "draft", -1: "disabled"}.get(value, "draft")


def _instance_status(value):
    return {1: "active", 0: "stopped", 2: "done"}.get(value, "active")


def _task_status(value):
    return {0: "pending", 1: "done", 2: "skipped"}.get(value, "pending")
...
def _ensure_step_access(cursor, instance_id, step_id, operator_id, can_manage=False):
    cursor.execute(
        """
        SELECT i.id
        FROM flow_instances i
...
def _map_template(row):
    return {
        ...
        "status": _template_status(row["status"]),
...
def _map_instance(row):
    ...
    return {
        ...
        "department": row["department_name"],
        "status": _instance_status(row["status"]),
        ...
        "updated": _dt(row.get("updated_at") or row.get("started_at")),
