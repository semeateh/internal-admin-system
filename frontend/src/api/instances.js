...
export function deleteInstance(instanceId) {
  return del(`/api/flow/instances/${instanceId}`).then(unwrap);
}
...
export function updateInstanceStepAssignment(instanceId, stepId, payload) {
  return put(`/api/flow/instances/${instanceId}/steps/${stepId}/assignment`, payload).then(unwrap);
}

export function recordStepRevision(instanceId, stepId, message) {
  return post(`/api/flow/instances/${instanceId}/steps/${stepId}/revision`, { message }).then(unwrap);
}

export function generateChecklist(instanceId) {
  return post(`/api/flow/instances/${instanceId}/checklist`).then(unwrap);
}
